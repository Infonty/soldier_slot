  ...
  <div class="collapse navbar-collapse" style="flex-grow:0;" id="navbarSupportedContent">
    ...
    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
      ...
      <li class="nav-item text-md-start text-center mx-2">
        <a class="nav-link {{request()->is('dashboard') ? 'active' : ''}}" href="{{route('dashboard')}}">QR-Scanner</a>
      </li>

      <li class="nav-item text-md-start text-center mx-2">
        <a class="nav-link {{request()->is('slot') ? 'active' : ''}}" href="{{route('slot')}}">Slots</a>
      </li>

      <li class="nav-item dropdown text-center">
        <form action="{{route('logout')}}" method="post">
        @csrf
        <button type="submit" class="rounded-pill btn btn-outline-primary px-3 fs-5 mx-md-4 mx-0 ">
            Logout
